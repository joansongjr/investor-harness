...
| "X 一图" / "一张图看 X" / "一页图" | 走 `sm-infographic`（v0.9.9）|
| "对比 X 和 Y 基金" / "基金对比" | 走 `sm-fund-compare`（v0.9.9）|
