...
0. **⛔ 置顶：用户本地资料优先**——任何外部取数之前，先扫描并调用用户本地电脑里已有的全部相关资料（手稿 / 笔记 / 规范 / 模板 / 模型 / 纪要 / harness 归档，见 preamble Step 2.0）。用户本地规范与要求是最高优先级的要求来源，高于 harness 默认方法论；本地已有研究是本次工作的起点，外部取数只做更新与验证
1. **先确认任务的市场归属**（A 股 / 港股 / 美股 / 跨市场），参见 [markets.md](markets.md)
2. **按优先级探测可用数据源**：本地资料 → MCP 工具 → skill 工具 → 通用搜索 → 兜底
3. **拿到什么数据就标什么完整中文证据等级**（见 [evidence.md](evidence.md)）
...
**优先级 2.5：Gangtise / AlphaPai（收盘复盘、日度归因、卖方线索增强时优先补）**

适用场景：
- `sm-close-recap`
- `sm-catalyst-sweep`
- 任何用户明确要求看**卖方小段子 / 研报 / 路演纪要 / 行业信息**的任务

推荐组合：

- `gangtise-agent` `security-clue-list`
  - 按证券 / 行业抓 `researchReport`、`conference`、`view`、`announcement`
  - 适合看**当日卖方线索、电话会、公告摘要**
- `gangtise-agent` `theme-tracking`
  - 适合补**当天行业 / 主题脉络**
- `gangtise-file` `report`
  - 适合定位最近 1-3 篇相关报告
- `gangtise-kb`
  - 适合直接读观点原文 / 段落
- `alphapai-research` `recall`
  - 适合召回 `comment,report,roadShow,qa,ann,wechat_public_article`
- `alphapai-research` `agent --mode 11`
  - 适合拉**行业一页纸**

使用原则：
- 这层是**基本面归因增强层**，不是替代官方公告层
- 如果与公告 / 财报冲突，以公告 / 财报为准
- 如果没有命中，也要明确写"未检索到相关卖方 / 行业线索"
...
- **要**为每条关键事实打完整中文证据等级标签（公开事实 / 财报披露 / 市场共识 / 合理推演 / 待核验假设）
