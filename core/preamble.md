...
- ❌ 证据分级（公开事实 / 财报披露 / 市场共识 / 合理推演 / 待核验假设）
- ❌ "仍需补的资料"段
- ❌ 合规声明
...
## Step 2 · 本地资料扫描 + 检查历史输出（治健忘 · ⛔ 置顶要求来源）

### Step 2.0 · 本地资料扫描（v0.9.6 置顶硬约束）

**在调用任何外部数据源之前**，先调用用户本地电脑里已有的全部相关资料，它们是**最高优先级的要求来源**：

1. **扫描范围**（按存在性逐项检查）：
   - 工作区内该标的 / 主题的**手稿、笔记、草稿**（不限于 harness 归档——用户自己写的任何文件）
   - 用户的**规范 / 方法论 / 模板文件**（如工作区内的写作标准、报告方法论、文档模板目录）
   - harness 归档（coverage/ themes/ briefings/，见下）
   - 用户本地的**模型 .xlsx / 纪要 / 已下载的公告财报**
2. **优先级规则**：
   - 用户本地规范与要求 **> harness 默认方法论 > 外部通用惯例**——冲突时按用户本地口径执行
   - 用户本地已有研究结论是本次工作的**起点**，不做重复劳动；外部取数用于**更新和验证**，不是从零重来
   - 事实性数据仍以公告 / 财报为准；本地手稿中的判断按其性质标证据等级
3. **交代义务**：[Preflight] 中必须列出本次扫描到的本地资料清单（或明确写"本地无相关资料"），禁止跳过扫描直接外部取数

### Step 2.1 · 检查 harness 历史输出
...
- 拿到的每条数据**立即**标完整中文证据等级（公开事实 / 财报披露 / 市场共识 / 合理推演 / 待核验假设，见 [evidence.md](evidence.md)）
