...
允许省略的：完整 13 段式 deepdive、历史输出检查、归档步骤
...
✅ **正确**：识别"估值怎么样"+ "KEYS / MU 是覆盖池成员"→ 触发 `sm-valuation`（两次；估值 skill，v0.9.5 新增。如无近期 deepdive 盈利预测则先跑 `sm-company-deepdive`）→ 完整 7 段估值输出 → postamble 归档。
...
❌ **违规**：调了 skill 但直接开始逐段输出，没写 `[Preflight]`。

✅ **正确**：调 skill → 先写 `[Preflight]`（标的/市场/数据源链/缺失项）→ 再按 skill 结构逐段输出。
