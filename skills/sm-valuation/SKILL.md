...
Valuation 特别注意：
- preamble Step 2 必须读取该公司最新 sm-company-deepdive（盈利预测量价拆分）输出；没有盈利预测就没有估值，缺失时先补 deepdive 或让用户提供预测
...
### §2.2 关键假设清单
盈利预测关键假设（引自 deepdive 量价拆分）、折现率 / 永续增速（如用 DCF）、每条标证据等级
...
| 关系 | 说明 |
|---|---|
| **依赖** | sm-company-deepdive（盈利预测量价拆分是估值的输入）|
